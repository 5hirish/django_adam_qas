...
def process_question(question, qclass, en_nlp):
    ...
    for token in sent:
        ...
        # print(wh_bi_gram)
        # print(root_token)
    with open(static_files+'/data/qclassifier_trainer.csv', 'a', newline='') as csv_fp:
...
def clean_old_data():
    with open(static_files+'/data/qclassifier_trainer.csv', 'w', newline='') as csv_fp:
        csv_fp_writer = csv.writer(csv_fp, delimiter='|')
...
def train_question_classifier():
    ...
    with open(static_files+'/data/qclassification_data.txt', 'r') as fp:
